...
fragment float4 test_passthrough_fs(
    VertexOutput in [[stage_in]],
    constant ArgumentBuffer& argBuf [[buffer(0)]]
) {
    // Reference argBuf so Metal compiler does not strip buffer(0) from
    // the fragment stage.  The backend binds argument buffers to both
    // vertex and fragment stages unconditionally; without this read the
    // debug layer asserts "unused binding at Buffer index 0".
    // colorMultiplier is {1,1,1,1} in all tests, so this is identity.
    return in.color * argBuf.ubo->colorMultiplier;
}
)MSL";
...
fragment float4 test_fill_fs(
    VertexOutput in [[stage_in]],
    constant FillArgumentBuffer& argBuf [[buffer(0)]]
) {
    return argBuf.ubo->fillColor;
}
)MSL";
