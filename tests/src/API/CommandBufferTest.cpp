    ...
    for (const auto& api : apis) {
        ...
        SNAP_RHI_TEST_BACKEND_SECTION(ctx->apiName) {
            ...
            SECTION("Submit and waitIdle") {
                snap::rhi::BufferCreateInfo srcBufInfo{};
                srcBufInfo.bufferUsage = snap::rhi::BufferUsage::TransferSrc;
                srcBufInfo.memoryProperties =
                    snap::rhi::MemoryProperties::HostVisible | snap::rhi::MemoryProperties::HostCoherent;
                srcBufInfo.size = 16;
                auto srcBuf = ctx->device->createBuffer(srcBufInfo);

                snap::rhi::BufferCreateInfo dstBufInfo{};
                dstBufInfo.bufferUsage = snap::rhi::BufferUsage::TransferDst;
                dstBufInfo.memoryProperties =
                    snap::rhi::MemoryProperties::HostVisible | snap::rhi::MemoryProperties::HostCoherent;
                dstBufInfo.size = 16;
                auto dstBuf = ctx->device->createBuffer(dstBufInfo);

                auto cmdBuf = ctx->device->createCommandBuffer({.commandQueue = ctx->commandQueue});
                auto* blit = cmdBuf->getBlitCommandEncoder();
                blit->beginEncoding();
                snap::rhi::BufferCopy region{.srcOffset = 0, .dstOffset = 0, .size = 16};
                blit->copyBuffer(srcBuf.get(), dstBuf.get(), {&region, 1});
                blit->endEncoding();
