...
public class SDLControllerManager {
    ...
    static native void nativeAddJoystick(int device_id,
                                         String name,
                                         String desc,
                                         int vendor_id,
                                         int product_id,
...
class SDLJoystickHandler {
    ...
    static class RangeComparator implements Comparator<InputDevice.MotionRange> {
        ...
        public int compare(InputDevice.MotionRange arg0, InputDevice.MotionRange arg1) {
            // Some controllers, like the Moga Pro 2, return AXIS_GAS (22) for right trigger and AXIS_BRAKE (23) for
            // left trigger - swap them so they're sorted in the right order for SDL
        ...
        for (int device_id : deviceIds) {
            if (SDLControllerManager.isDeviceSDLJoystick(device_id)) {
                ...
                if (joystick == null) {
                    ...
                    SDLControllerManager.nativeAddJoystick(joystick.device_id,
                                                           joystick.name,
                                                           joystick.desc,
        ...
        if (action == MotionEvent.ACTION_MOVE) {
            ...
            if (joystick != null) {
                ...
                for (int i = 0; i < joystick.hats.size() / 2; i++) {
                    ...
                    int hatY =
                        Math.round(event.getAxisValue(joystick.hats.get(2 * i + 1).getAxis(), actionPointerIndex));
            ...
            if (toolType == MotionEvent.TOOL_TYPE_MOUSE) {
                ...
                switch (action) {
                    ...
                    case MotionEvent.ACTION_HOVER_EXIT:
                        ...
                        int buttons =
                            (event.getButtonState() >> 4) | (1 << (toolType == MotionEvent.TOOL_TYPE_STYLUS ? 0 : 30));
                        if ((event.getButtonState() & MotionEvent.BUTTON_TERTIARY) != 0) {
                            buttons |= 0x08;
                        }

                        SDLActivity.onNativePen(
                            event.getPointerId(i), getPenDeviceType(event.getDevice()), buttons, action, x, y, p);
                        consumed = true;
                        break;
                }
            }
